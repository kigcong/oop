...
public class HanaBank {
	public static void main(String[] args) {
		...
		while (true) {
			System.out.println("1.계좌개설 2.계좌조회 3.계좌조회(이름) 4.전체계좌 0.종료");
			input = scanner.nextInt();
			// 무조건 메소드를 호출하면 이클립스가 자동으로 메소드를 만들어주는 create method 메뉴가 나타나고
			// 그것을 이용해서 새로 메소드를 나타낸다
			...
			if (input != 0) {
				...
				case 3:
					...
					for (BankBook bankBook2 : searchArr) { // : in으로 생각
			...
			if (input != 0) {
				...
				break; // 무란루프 로직에서 시스템 종료요청시
				// 조건에 합강하면 break 처리로 나감
				// switch caes 에서도 동일하게 사용
			}
	...
	private static int inputCheck(int input, int i, int j) {
		if (input < i || input > j) {
			...
			return 0; //시슴템 종료 .. 리턴차입의 디폴트값을 리턴했기 때문
		} else {
			return input;
		}
